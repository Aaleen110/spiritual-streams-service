...
// .wrangler/tmp/bundle-f6AckQ/checked-fetch.js
...
async function streamAudioFromR2BL(c, db, audioBucket, inputs) {
  const { id } = inputs;
  console.log("Streaming audio for part ID:", id);
  const part = await db.select().from(sermonParts).where(eq(sermonParts.id, id)).limit(1);
  if (!part.length) {
    throw new NotFoundError("Sermon part not found");
  }
  const sermonPart = part[0];
  console.log("Found part:", sermonPart);
  const r2Key = sermonPart.audioUrl;
  console.log("Looking for file in R2 with key:", r2Key);
  ...
  console.log("Found audio object in R2, size:", audioObject.size);
  ...
  console.log("Returning full file");
...
// .wrangler/tmp/bundle-f6AckQ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-f6AckQ/middleware-loader.entry.ts
