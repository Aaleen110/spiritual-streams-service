...
// .wrangler/tmp/bundle-aibcTu/checked-fetch.js
...
async function streamAudioFromR2BL(c, db, audioBucket, inputs) {
  const { id } = inputs;
  const part = await db.select().from(sermonParts).where(eq(sermonParts.id, id)).limit(1);
  if (!part.length) throw new NotFoundError("Sermon part not found");
  const sermonPart = part[0];
  const r2Key = sermonPart.audioUrl;
  const audioObject = await audioBucket.get(r2Key);
  if (!audioObject) throw new NotFoundError("Audio file not found");
...
// .wrangler/tmp/bundle-aibcTu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-aibcTu/middleware-loader.entry.ts
