...
// .wrangler/tmp/bundle-uKi8gz/checked-fetch.js
...
__export(schema_exports, {
  sermonParts: () => sermonParts,
  sermons: () => sermons
});
var sermons = sqliteTable("sermons", {
...
var sermonParts = sqliteTable("sermon_parts", {
  id: text("id").primaryKey(),
...
var ResponseUtility = class {
  static {
    __name(this, "ResponseUtility");
  }
...
// src/utils/paramutil.ts
async function processInputs(c, schema) {
  const inputs = {};
  for (const [field, [required, defaultValue, source]] of Object.entries(schema)) {
    ...
    inputs[field] = value !== void 0 ? value : defaultValue;
  }
  return inputs;
}
...
// src/api/v1/io.ts
async function getAllSermons(db, page = 1, limit = 10, search) {
  const offset = (page - 1) * limit;
  const sermonsQuery = search ? db.select().from(sermons).where(like(sermons.title, `%${search}%`)) : db.select().from(sermons);
...
__name(getAllSermons, "getAllSermons");
async function getSermonById(db, id) {
...
__name(getSermonById, "getSermonById");
async function getSermonParts(db, sermonId) {
  const parts = await db.select().from(sermonParts).where(eq(sermonParts.sermonId, sermonId)).orderBy(asc(sermonParts.partNumber));
  return parts.map((part) => ({
    ...part,
    audioUrl: `https://pub-f93cfefd9ddd424ab1cfd836a017d797.r2.dev/${part.audioUrl}`
  }));
}
__name(getSermonParts, "getSermonParts");
async function getSermonWithParts(db, id) {
...
__name(getSermonWithParts, "getSermonWithParts");
async function getSermonPartById(db, id) {
  ...
  return {
    ...part[0],
    audioPath: part[0].audioUrl,
    // relative path for backend
    audioUrl: `https://pub-f93cfefd9ddd424ab1cfd836a017d797.r2.dev/${part[0].audioUrl}`
    // public URL for frontend
  };
}
__name(getSermonPartById, "getSermonPartById");

// src/api/v1/bl.ts
async function getAllSermonsBL(c, db, inputs) {
  const { page = 1, limit = 10, search } = inputs;
  return await getAllSermons(db, page, limit, search);
}
__name(getAllSermonsBL, "getAllSermonsBL");
async function getSermonByIdBL(c, db, inputs) {
  const { id } = inputs;
  return await getSermonWithParts(db, id);
}
__name(getSermonByIdBL, "getSermonByIdBL");
async function getSermonPartsBL(c, db, inputs) {
  const { sermonId } = inputs;
  return await getSermonParts(db, sermonId);
}
__name(getSermonPartsBL, "getSermonPartsBL");
async function getSermonPartByIdBL(c, db, inputs) {
  const { id } = inputs;
  return await getSermonPartById(db, id);
}
__name(getSermonPartByIdBL, "getSermonPartByIdBL");
async function streamAudioBL(c, db, inputs) {
  const { id } = inputs;
  const part = await getSermonPartById(db, id);
  return {
...
__name(streamAudioBL, "streamAudioBL");
async function streamAudioFromR2BL(c, db, audioBucket, inputs) {
  const { id } = inputs;
  const part = await getSermonPartById(db, id);
  const audioObject = await audioBucket.get(part.audioPath);
  if (!audioObject) {
  ...
  if (rangeHeader) {
    ...
    if (range) {
      ...
      setRangeHeaders(c, range, fileSize, "audio/mpeg");
      c.status(206);
  ...
  setFullFileHeaders(c, fileSize, "audio/mpeg");
...
__name(streamAudioFromR2BL, "streamAudioFromR2BL");
...
function setRangeHeaders(c, range, fileSize, mimeType) {
...
function setFullFileHeaders(c, fileSize, mimeType) {
  c.header("Accept-Ranges", "bytes");
  c.header("Content-Length", fileSize.toString());
  c.header("Content-Type", mimeType);
}
__name(setFullFileHeaders, "setFullFileHeaders");

// src/api/v1/sermons.ts
var getAllSermons2 = /* @__PURE__ */ __name(async (c) => {
  try {
    const inputs = await processInputs(c, {
      page: [false, 1, "query"],
      limit: [false, 10, "query"],
      search: [false, void 0, "query"]
    });
    const db = c.get("db");
    const result = await getAllSermonsBL(c, db, inputs);
...
var getSermonById2 = /* @__PURE__ */ __name(async (c) => {
  try {
    const inputs = await processInputs(c, {
      id: [true, void 0, "param"]
    });
    const db = c.get("db");
    const result = await getSermonByIdBL(c, db, inputs);
...
var getSermonParts2 = /* @__PURE__ */ __name(async (c) => {
  try {
    const inputs = await processInputs(c, {
      sermonId: [true, void 0, "param"]
    });
    const db = c.get("db");
    const result = await getSermonPartsBL(c, db, inputs);
...
// src/api/v1/parts.ts
var getSermonPartById2 = /* @__PURE__ */ __name(async (c) => {
  try {
    const inputs = await processInputs(c, {
      id: [true, void 0, "param"]
    });
    const db = c.get("db");
    const result = await getSermonPartByIdBL(c, db, inputs);
...
var streamAudio = /* @__PURE__ */ __name(async (c) => {
  try {
    const inputs = await processInputs(c, {
      id: [true, void 0, "param"]
    });
    const db = c.get("db");
    const result = await streamAudioBL(c, db, inputs);
...
// src/api/v1/stream.ts
var streamAudioFile = /* @__PURE__ */ __name(async (c) => {
  try {
    const inputs = await processInputs(c, {
      id: [true, void 0, "param"]
    });
    ...
    const result = await streamAudioFromR2BL(c, db, audioBucket, inputs);
    return result;
  } catch (error) {
    if (error instanceof ValidationError) {
      return c.json(ResponseUtility.badRequest(error.message), 400);
...
var api = new Hono2();
api.get("/sermons", getAllSermons2);
api.get("/sermons/:id", getSermonById2);
api.get("/sermons/:sermonId/parts", getSermonParts2);
api.get("/parts/:id", getSermonPartById2);
api.get("/parts/:id/stream", streamAudio);
api.get("/audio/:id", streamAudioFile);
...
// src/index.ts
var src_default = router_default;
...
// .wrangler/tmp/bundle-uKi8gz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-uKi8gz/middleware-loader.entry.ts
